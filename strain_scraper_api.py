...
def get_strain_data_from_ai(strain_name):
    ...
    - Terpenes (array format):
    - Benefits (array format, no medical claims):
    ...
    try:
        ...
        for line in lines:
            if line.startswith("Name:"):
                ...
                structured_data["effects"] = [e.strip() for e in line.split(":", 1)[1].split(",")]
            elif line.startswith("Aromas:"):
                structured_data["aromas"] = [a.strip() for a in line.split(":", 1)[1].split(",")]
            elif line.startswith("Flavors:"):
                structured_data["flavors"] = [f.strip() for f in line.split(":", 1)[1].split(",")]
            elif line.startswith("Terpenes:"):
                structured_data["terpenes"] = [t.strip() for t in line.split(":", 1)[1].split(",")]
            elif line.startswith("Benefits:"):
                structured_data["benefits"] = [b.strip() for b in line.split(":", 1)[1].split(",")]
            elif line.startswith("Sources:"):
                structured_data["sources"] = [s.strip() for s in line.split(":", 1)[1].split(",")]
